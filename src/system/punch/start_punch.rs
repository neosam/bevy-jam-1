...
pub fn start_punch(
    ...
    mut query: Query<(Entity, &mut component::Punch, &Transform, &component::Side), (With<component::Paddle>, Without<component::ColliderOnly>, Without<component::PunchAnimation>)>,
    mut start_punch_events: EventReader<crate::StartPunchEvent>,
) {
    for event in start_punch_events.iter() {
        for (entity, mut punch, transform, _) in query.iter_mut()
