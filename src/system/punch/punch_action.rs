...
pub fn punch_action(
    ...
    paddle_query: Query<(&Transform, &component::Side, &component::Punch), (With<component::Paddle>, Without<component::ColliderOnly>)>,
    mut punch_event_reader: EventReader<crate::StartPunchEvent>,
) {
    for event in punch_event_reader.iter() {
        for (paddle_transform, _, punch) in paddle_query.iter().filter(|(_, side, punch)| event.side == **side && punch.strength >= 1.0) {
            for (mut velocity, ball_transform) in ball_query.iter_mut()
                    .filter(|(_, ball_transform)| (paddle_transform.translation - ball_transform.translation).length() <= punch.strength * 200.0) {
